...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self) -> QWidget:
        ...
        brand_subtitle = QLabel("Windows 预览版")
    ...
    def _build_top_bar(self) -> QWidget:
        ...
        self.help_button.clicked.connect(
            lambda: self._show_placeholder_message("当前版本重点先完成下载主流程和 Windows 桌面界面。")
        )
    ...
    def _poll_system_theme(self) -> None:
        ...
        if self.config.theme_mode == "system":
            self._apply_styles()
            self._push_log(f"已跟随 Windows 系统主题切换为：{_theme_label(self.effective_theme_mode)}。")
    ...
    def _set_window_icon(self) -> None:
        icon_path = _resolve_asset_path("app_icon.ico")
        if icon_path.exists():
